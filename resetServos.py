...
DXL1_END_POSITION_VALUE = 475   # Closed values # OLD Values: 825 950
DXL2_END_POSITION_VALUE = 500
...
# -- Servo functions --
def set_servo(angle_deg, fingerIdx): # PWM 2 == thumb, PWM 0 == middle
    ...
    print(angle_deg, fingerIdx, sep=",  ")
    return angle_deg

def close_hand():	# we need to find this_angle then add angles to squeeze the object. (don't use LEFT_DUTY_NS)
    #write(f"{pwm_base}/pwm0/duty_cycle", SERVO_RIGHT_DUTY_NS)
    #write(f"{pwm_base2}/pwm0/duty_cycle", SERVO_RIGHT_DUTY_NS)
    set_servo(80, 2)
    set_servo(90, 0)
...
# -- Main runner --
async def main():
    # Test both full motions
    open_hand()
    time.sleep(5)
    close_hand()
    time.sleep(5)
    # Run async tasks, sensing and motor control
